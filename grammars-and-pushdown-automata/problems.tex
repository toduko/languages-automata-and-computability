...
Упътване: използвайте \thref{non-cf-intersect-with-regular} като пресичате $\{ x_1 \}^* \{ x_2 \}^* \{ x_3 \}^* \{ x_4 \}^*$ с някой безконтекстен език, подобен на езика от \thref{a^nb^ka^nb^k}.
\end{problem}