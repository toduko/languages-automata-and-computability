...
\begin{definition}
    ...
    Нека $L \subseteq \Sigma^*$. Тогава:
    \begin{itemize}
        \item $Pref(L) = \{ \alpha \in \Sigma^* \: | \: (\exists \beta \in L)(\alpha \preceq_{pref} \beta) \}$
        \item $Suff(L) = \{ \alpha \in \Sigma^* \: | \: (\exists \beta \in L)(\alpha \preceq_{suff} \beta) \}$
        \item $Infix(L) = \{ \alpha \in \Sigma^* \: | \: (\exists \beta \in L)(\alpha \preceq_{inf} \beta) \}$
    \end{itemize}
\end{definition}
...
Упътване: за първото да се направи индукция по $\alpha$, а за второто да се използва първото
\end{problem}
\begin{problem}
Да се докаже, че:
\begin{itemize}
    \item $Pref(L_1 \cdot L_2) = Pref(L_1) \cup (L_1 \cdot Pref(L_2))$
    \item $Suff(L_1 \cdot L_2) = Suff(L_2) \cup (Suff(L_1) \cdot L_2)$
    \item $Infix(L_1 \cdot L_2) = Infix(L_1) \cup Infix(L_2) \cup (Suff(L_1) \cdot Pref(L_2))$
    \item $Infix(L) = Pref(Suff(L)) = Suff(Pref(L))$
\end{itemize}
